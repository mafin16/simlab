...
| 2026-08-24 | Verifikasi ulang unduh PDF Laporan & Audit setelah laporan user "tombol PDF mati sementara Excel normal". Forensik log: error asli adalah TypeError null-return di `maintenancePdf()` pada 21 Agu 12:56 — terjadi lewat **sesi `artisan serve`** (stack trace memuat `Foundation/resources/server.php`) SEBELUM commit finalisasi jam 14:03; fix (`$response = $pdf->download(...)` + `return $response`) sudah termuat di `830515f`, sehingga **tidak ada perubahan kode tambahan** yang diperlukan. Kesan "masih rusak" merupakan sisa pengalaman pra-fix. Verifikasi 24 Agu: OPcache nonaktif + tanpa cache config/route (kode eksekusi = kode committed); curl login admin via Apache localhost:8080 → `maintenance/pdf` 200 @ 878.365 B (`%PDF-1.7`, 1,62 s), `presence/pdf` 200 @ 882.186 B (0,71 s), `presence/excel` 200; klik browser asli user (Chrome 151, referer `/reports`) mengunduh kedua PDF utuh 200; `laravel.log` nol entri baru. ReportTest 8 pass (39 assertions). Catatan operasional: request IPv6 (`::1`) tercatat di `access.log` utama sedangkan IPv4 (`127.0.0.1`) masuk `simlab-access.log` — keduanya menyajikan app yang sama, bukan indikasi salah route. |